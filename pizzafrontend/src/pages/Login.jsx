...
function Login () {
  ...
  return(
    <form onSubmit={onSubmitHandler} className="flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800" action="">
      ...
      {currentState === 'Login' ? '' :   <input type="text" className="w-full px-3 px-2 border border-gray-800" placeholder="Név" required/>}
      <input type="email" className="w-full px-3 px-2 border border-gray-800" placeholder="Email" required/>
      <input type="password" className="w-full px-3 px-2 border border-gray-800" placeholder="Jelszó" required/>
      ...
      <button className="bg-black text-white font-light px-8 py-2 mt-4">{currentState === 'Login' ? 'Sing In' : 'Sing In'}</button>
    </form>
